...
filter_elf(){
	...
	for _elf_f in "${ABELFFILTERS[@]}"; do _elf_cmd=filter_elf_${_elf_f}_pre; ! _which "$_elf_cmd" &>/dev/null || "$_elf_cmd" || abwarn "$(argprint "$_elf_cmd" "$@"): $?"; done
	...
	cd "$PKGDIR"
	for _elf_f in "${ABELFFILTERS[@]}"; do _elf_cmd=filter_elf_${_elf_f}_post; ! _which "$_elf_cmd" &>/dev/null || "$_elf_cmd" || abwarn "$(argprint "$_elf_cmd" "$@"): $?"; done
	rec_opt nullglob
}
